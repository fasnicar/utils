...
try:
    ...
    print "py shared_bugs.py <tsv-mp2-merge> <colX>,<colY>,.. <output-name>"
...
with open(tsv) as f:
    for r in f:
        if header:
            ...
            for c in cols:
                labels.append(r.strip().split()[c-1])
        else:
            bug = str(r.strip().split()[0])
            bugs_list[bug] = []

            for c in cols:
                bugs_list[bug].append(float(r.strip().split()[c-1]))
